...
STEP 4 — PROJECT RULES — driven by `RULE_REGISTRY.md`:
  ...
  ☐ RE-RUN ON EVERY PHASE TRANSITION — this step is NOT bootstrap-only. Whenever a skill activates a NEW phase mid-session (per `<phase_definitions>` in RULE_REGISTRY.md — e.g. `brainstorming` → `writing-plans` → `executing-plans`), or the stack/domain scope changes, recompute `expected_rules` for the new phase/stack/domain and READ every missing rule BEFORE executing that skill's workflow. A session that bootstrapped at `<none-yet>` and never reloads rules when planning or implementation begins is a workflow violation — the four core phase skills (`brainstorming`, `writing-plans`, `executing-plans`, `subagent-driven-development`) carry their own PHASE RULE SYNC step as a backstop, but this contract is canonical here and applies to EVERY phase in `<phase_definitions>`, including those whose skills carry no backstop (e.g. `test-driven-development`, `systematic-debugging`, the review and shipping skills).
...
<workflow_retrieval_gates>
These fire on WORKFLOW moments, not question-semantics. All are SOFT / best-effort (no hard rule;
no hook). They are additive to FAMILY A/B/C above.

GATE 1 — TASK INTAKE: on `brainstorming` phase entry (or first message naming a feature area),
run Tier-3 (`_SEARCH_INDEX.md`) + Tier-1b (`query-qdrant.py --collection wiki_pages`) ONCE, scoped
to the area, before raw `ls`/`grep` recon. Ad-hoc work: scope to the nouns in the request.
...
GATE 3 — FIND CODE BY BEHAVIOR: when you do NOT know the symbol name and want to FIND code by
behavior/concept, run `query-qdrant.py --collection code_chunks` BEFORE a guessed/blind `grep`.
(Symbol name KNOWN → that's FAMILY B: graph first. Exact-token grep stays first-class.)
</workflow_retrieval_gates>

HARD RULE — for any FAMILY A match: Tier-1 is **best-effort** — query when `chat_history` has content; when it is known-empty at bootstrap, skip the FAMILY A route. For any FAMILY B match: Tier-2 query is MANDATORY before any `Read` or `grep`. For any FAMILY C match: Tier-3 (`_SEARCH_INDEX.md`) is MANDATORY before answering from training knowledge or a blind `grep`/`Read`, escalating to Tier-1b (`wiki_pages`) when Tier-3 misses or the question is conceptual. For HYBRID questions matching more than one family (e.g., "why did we choose Adjacency List in Sprint 15, and which services does the Comment entity affect"): run the matching tiers (in parallel where independent), then synthesize. PRECEDENCE when a question could match more than one: a code SYMBOL's structure/behavior ("what does ServiceX do") is FAMILY B (run Tier-2 first; escalate to C's Tier-1b only if the graph doesn't answer); reserve FAMILY C for project facts/conventions/how-to that aren't a named symbol. When NO symbol name is known and the goal is to FIND code by behavior, that is GATE 3 (`code_chunks` first), not FAMILY B. Skipping a matching tier and going straight to file/history reads is a workflow violation.
...
STEP 1A — Tier-1 query (Qdrant `chat_history`, best-effort for FAMILY A):
...
STEP 2 — Evaluate result(s):
  ...
  - COLD-START (FAMILY A): if Tier-1 is empty or unreachable — no `chat_history` yet (a fresh adoption: summaries are ingested at handoff / branch-finish), or Qdrant down — note it **once per session**, then suppress the FAMILY A route for the rest of the session ("no session history yet") and continue; do NOT re-run the empty query.
...
DEVELOPMENT GATE (eager — applies even when no skill is loaded yet) — DO NOT write or modify project source code unless one of these holds: (a) an implementation-phase skill (`executing-plans`, `subagent-driven-development`, `test-driven-development`) is active and working from an approved plan; (b) `systematic-debugging` is active for a bug fix, or the change is directed by an active review/optimization skill (`receiving-code-review`, `performance-optimization`); or (c) the user explicitly asked for a trivial direct edit (typo/config one-liner) and you confirmed it needs no design. A request for a new feature, system, refactor, or architectural change routes FIRST to `brainstorming` — no matter how detailed the user's task description is. A detailed description is the INPUT to the spec, not a substitute for it; "the user already told me exactly what to build" is a forbidden rationalization. Answering a question with an example snippet is NOT gated (no project file is modified), and code embedded in spec/plan artifacts is plan content, not implementation. Focused task subagents: the dispatching orchestrator's workflow satisfies this gate — execute your payload. If you are about to write or modify project source code and none of (a)-(c) holds, that is a dispatch failure — re-run `using-superpowers` STEP 4 instead of continuing.

SECURITY MANDATE — if the task involves planning, writing, or reviewing code, load `secure-coding-standard` BEFORE drafting the plan or the code — `brainstorming` and `writing-plans` count as "planning code", so the load happens at spec/plan time, not first-code time. This is unconditional for code work; touching user input, auth, DB queries, file uploads, secrets, CORS, rate limiting, or any external/untrusted data merely makes it even less skippable.
...
When triggered, complete the smallest meaningful unit first, then run the assessment. Always update Second Brain (active-project-context + session-summary), run the skill's `<decision_triage>` so this session's decisions reach a durable home rather than dying in the chat. MOST of them are TASK-SCOPED constraints — what is out of scope, which areas are off-limits, what is deferred to another team — and those live in `## Task Ground Rules` in the plan artifact and the session summary, carried in every handoff until lifted; losing one leaves no visible gap, it produces an agent doing work it was told not to do, so `executing-plans` reads them before writing any code. Project-level decisions route to `architecture-decisions.md` via `using-second-brain` operation 5, and decisions about how the AGENT works to `_improvement-queue/` via a `decision`/`preference` signal, and run `/evolve` pre-check before generating the handoff prompt. `context-handoff` NEVER writes `architecture-decisions.md` itself — it routes to the workflow that owns the consent gate and AD numbering. The handoff also carries EXPERIENCE forward — dead ends, tooling quirks, how this user works — in the session summary's `## Notes for the Next Session` and the prompt's Agent Notes, pruned each handoff. That channel is deliberately NOT the `/evolve` queue: a captured signal stays `pending` until `/evolve` runs and the next session is shown only its COUNT, so filing experience as a signal to move it across a session boundary removes it from circulation. `finishing-a-development-branch` rolls the generalizable notes up into signals in one batch at task end. If context is too tight to finish, follow the skill's `<minimum_viable_handoff>` ladder instead of dropping steps arbitrarily. Always confirm with the user before switching modes.
