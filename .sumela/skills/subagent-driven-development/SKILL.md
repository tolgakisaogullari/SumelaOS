...
4. QUALITY & SECURITY REVIEW (STAGE 2 — Checkpoint mode): Dispatch Code Quality Reviewer subagent (`./code-quality-reviewer-prompt.md`).
   ...
   - INDEPENDENT REVIEW OPTION: If the user selects option (2), dispatch an independent review subagent using `requesting-code-review`'s `code-reviewer.md` template, filling its payload per that skill's **Step 4** (an unfilled `{PLACEHOLDER}` reaches the subagent as literal text) If it finds issues: dispatch Implementer to fix, re-stage, re-dispatch until approved, then re-present the 4 approval options. This is in addition to the mandatory Stage-1/Stage-2 reviews — a third-party check when the user wants extra assurance.
