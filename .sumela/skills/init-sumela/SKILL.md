...
- **PROVE integrity after import**: `python3 scripts/reconcile-registry.py --check` AND `bash scripts/validate-structure.sh --post-setup` MUST pass. No Python 3 → no reconcile: check registry↔frontmatter parity BY HAND, never report "proven". These prove the SKILL side, domain-conditional rule↔file parity, and repo hygiene. They do NOT deep-validate stack rows or the matrix (reconcile covers only domain rule files), so for each imported stack rule ALSO eyeball its matrix row (valid `stack=`/`activation=`, real phase). If anything fails, fix or revert it before continuing.
