...
7. MEMORY PERSISTENCE (SECOND BRAIN - CODE-COMMIT INGEST CHECKLIST):
   ...
   - Execute this 8-item CODE-COMMIT INGEST checklist in order. Skipping any applicable item is a violation:
     ...
     4. **Core: `architecture-decisions.md`** - if any architectural decisions were made (new pattern, library swap, layer change), add a new AD entry using the **Entry shape** documented at the top of that page (`## AD-XX`, then `**Status:** accepted` / `**Date:**` / `**Superseded by:** —`, then Decision → Context → Alternatives → Outcome).
        - **Repair the page first** — the same three checks as `using-second-brain` operation 5, done mechanically with NO consent prompt (that ASK governs capturing a DECISION mid-conversation, not materializing the page, and this item is already mandatory and non-interactive):
          - **Absent** → create it from `docs/second-brain/template/wiki/architecture-decisions.md.template`, substituting `{{project_name}}` / `{{date_created}}`, and add its `_INDEX.md` + `_SEARCH_INDEX.md` rows.
          - **Present but unlinked** → add those index rows.
          - **Present but in the pre-0.18 shape** (`## AD-` entries, no `## Decisions` section, no per-entry `**Status:**`) → run the one-time migration in operation 5: wrap the existing entries in the `## Conventions` + Entry-shape + `## Decisions` scaffold, keep every entry byte-identical, and drop the now-exempt `decision_id`/`decision_status` page frontmatter.
        - **Then promote what a `context-handoff` deferred.** `AD candidate (deferred)` lines are promotable — the user was never asked. Two markers are NOT: `AD declined` means the user was asked and refused, and the v0.18.0 spelling `AD candidate (not yet filed)` covered BOTH refusal and deferral, so neither can be written from this non-interactive step without risking turning a refusal into a silent write. Leave those in place and tell the user they need a decision, which operation 5's ask will take.
   ...
   - **EXPERIENCE ROLL-UP (batch, end of task).** Gather the `## Notes for the Next Session` sections from
     this task's session summaries — including notes earlier handoffs carried forward. Most are
     task-local and die here, correctly. For the ones that GENERALIZE beyond this task (a tooling
     quirk anyone will hit, an approach that reliably fails, a standing way this user wants to work),
     file them as `/evolve` signals in ONE batch per `<signal_capture>` rules, then say how many you
     filed. Do this HERE, not at handoff: a signal sits `pending` until `/evolve` runs and the next
     session is shown only its count, so filing mid-task would take the experience OUT of circulation
     instead of passing it on. Handoff carries experience forward; this step decides what becomes
     permanent. If nothing generalizes, say so and file nothing — an empty roll-up is a valid result.
