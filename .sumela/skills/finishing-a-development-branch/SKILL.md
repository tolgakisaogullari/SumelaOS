...
7. MEMORY PERSISTENCE (SECOND BRAIN - CODE-COMMIT INGEST CHECKLIST):
   ...
   - Execute this 8-item CODE-COMMIT INGEST checklist in order. Skipping any applicable item is a violation:
     ...
     4. **Core: `architecture-decisions.md`** - if any architectural decisions were made (new pattern, library swap, layer change), add a new AD entry using the **Entry shape** documented at the top of that page (`## AD-XX`, then `**Status:** accepted` / `**Date:**` / `**Superseded by:** —`, then Decision → Context → Alternatives → Outcome). Before appending, repair the page the same way `using-second-brain` operation 5 does — check BOTH conditions: if the page is **absent** (an install that upgraded into this version), create it from `docs/second-brain/template/wiki/architecture-decisions.md.template`, substituting the project name and today's date for its `{{project_name}}` / `{{date_created}}` placeholders; if it is **present but unlinked** (a project that hand-made one pre-0.18), add its `_INDEX.md` + `_SEARCH_INDEX.md` rows. Do both mechanically, with NO consent prompt: operation 5's *"save this decision to the wiki?"* ASK governs capturing a DECISION in a mid-session conversation, not materializing the container page, and this checklist item is already mandatory and non-interactive. Also promote any **AD candidate** lines a `context-handoff` left behind — that skill defers AD writes to here on purpose.
